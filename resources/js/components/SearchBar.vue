<template>
    <div>
        <div v-if="focus" @click="focus=false"
             class="bg-black opacity-0 absolute right-0 top-0 bottom-0 left-0 z-10"></div>
        ...
        <div class="relative z-10">
            ...
            <input type="text"
                   ...
                   placeholder="Search..." id="serachTerm" v-model="searchTerm"
                   @input="search" @focus="focus=true">

            <div v-if="focus" class="absolute bg-blue-900 text-white rounded-lg p-4 w-96 right-0 mr-6 mt-2 shadow z-20">
                <div v-if="!results.length || searchTerm.length < 3">No results found for {{searchTerm}}</div>
                <div v-else v-for="result in results" @click="focus=false">
